import { Contract, nativeToScVal, TransactionBuilder, rpc, scValToNative } from '@stellar/stellar-sdk';
...
var StellarVenue = class {
  ...
  async view(method, args) {
    ...
    const tx = new TransactionBuilder(account, { fee: this.baseFee, networkPassphrase: this.passphrase }).addOperation(this.contract.call(method, ...args)).setTimeout(30).build();
  ...
  async invoke(signer, op) {
    ...
    const tx = new TransactionBuilder(account, { fee: this.baseFee, networkPassphrase: this.passphrase }).addOperation(op).setTimeout(60).build();
...
export { StellarVenue, stellarOrderRef };
