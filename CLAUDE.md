...
> - **v0.50.0 (sortable collection tokens)**: `getCollectionTokens(contract, page?, limit?, sort?)` — new 4th param `CollectionTokensSort` (`"recent" | "oldest" | "name"`, default `"recent"`). Backend default order changed from `tokenId asc` to `createdAt desc` to match. Additive.
> - **v0.51.0 (price sort for collection tokens)**: `CollectionTokensSort` gains `"price"` — cheapest active listing first, unlisted tokens last. Additive.
...
> - **v0.52.0 (platform federation Phase A — chain-neutral core, BREAKING-staged)**: core no longer depends on starknet.js; the entire Starknet surface (MedialaneClient, marketplace modules, services, SIWS, admin-auth, Cairo ABIs, buildFeeCall, encodeByteArray, SNIP-12 builders) moved to **`@medialane/sdk/starknet`** (an equal chain adapter; root keeps deprecated re-exports for one transition window — migrate imports). New: `STELLAR` in `CHAINS` + strkey codec branch in `normalizeAddress`; per-chain-shaped coordinates (`StarknetCoordinates`/`EvmCoordinates`/`SolanaCoordinates`/`StellarCoordinates`, `getStarknetCoordinates()` guard; `ChainCoordinates` is a deprecated alias); chain-neutral `VenueAdapter`/`IssuanceAdapter` interfaces (`src/adapters/`); cross-chain read filter `chain?: Chain | "all"` on `getCollections`/`getOrders`/`getActivities`/`getCoins`/`search`. `starknet` peer is `peerDependenciesMeta`-optional. Spec: `medialane-core/docs/specs/2026-07-07-platform-multichain-federation-design.md`. NOT published.

> - **v0.52.0 addendum (Phases C–E adapters)**: `@medialane/sdk/evm` (viem, EIP-712 byte-verified vs the audited Solidity, EvmVenue/EvmIssuance), `@medialane/sdk/solana` (Anchor wire encoding without the Anchor client — discriminators/BorshWriter/PDAs, SolanaVenue), `@medialane/sdk/stellar` (Soroban invocation builders, StellarVenue, `stellarOrderRef` canonical ids). All optional peers (viem / @solana/web3.js / @stellar/stellar-sdk); all implement the chain-neutral `VenueAdapter`/`IssuanceAdapter`. NOT published.
> - **v0.56.0 (StarknetVenue 1155 partial-fill fix)**: `StarknetVenue.fulfillOrder` now computes the total as **per-unit price × quantity** (was passing the resolved amount straight through, which under-approved multi-edition ERC-1155 buys where `quantity > 1`; ERC-721 unaffected). `ResolvedOrder.totalPrice` → **`unitPrice`** (per-unit, raw) — the adapter multiplies by `quantity`. Matches the app's `orderTotal()` = per-edition × quantity semantics. Bug was latent in 0.55.0; caught during the Phase 2 wiring before any app consumed it.
> - **v0.55.0 (Starknet first-class venue adapter — Stage 0 app port)**: (published as 0.55.0, not 0.53.0 — npm `0.53.0`/`0.54.0` were foreign/regressed publishes from a stale package.json **missing the subpath `exports` map**, no `StarknetVenue`; 0.55.0 supersedes them and restores a working `latest`.) new **`StarknetVenue`** (`@medialane/sdk/starknet`) implements the chain-neutral `VenueAdapter<AccountInterface>` by **wrapping** the maintained `MarketplaceModule`/`Medialane1155Module` (no re-implementation of SNIP-12 signing or calldata) — so Starknet is a peer adapter alongside EVM/Solana/Stellar, and `medialane-starknet` can consume one protocol boundary. Dispatches 721 vs 1155 via an injected `resolveStandard` (the indexer already tracks it — no on-chain ERC-165 probe, which Starknet lacks a canonical id for). `registerOrder` maps the chain-neutral params (raw per-unit `amount` → the module's human price via `formatAmount`; new optional `RegisterOrderParams.quantity` for 1155 units) and returns the **contract-emitted `OrderCreated` hash** (`keys[1]`, exactly what the indexer stores) as `orderRef`, parsed from the tx receipt. `fulfillOrder`/`cancelOrder` take an `orderRef` + an injected `resolveOrder(orderRef)` read. `RegisterOrderParams` gained optional `quantity` (additive). **`StarknetIssuance` deferred** — the app's mint path is the backend-signed `createMintIntent` flow, not `MarketplaceModule.mint`, so the issuance adapter lands when that flow is refactored. Spec/plan: `medialane-core/docs/{specs,plans}/2026-07-09-starknet-first-class-adapter-and-app-port*`.
