...
> - **v0.41.0 (BREAKING — killed the `*_MAINNET` constant debt)**: removed **all** flat `*_MAINNET` address / class-hash / start-block exports from `constants.ts` + `index.ts`. **`getCoordinates(chain)` (`chains.ts`) is now the sole source** of every contract address. Migrate `X_MAINNET` → `getCoordinates(chain).<field>` (e.g. `MARKETPLACE_721_CONTRACT_MAINNET` → `getCoordinates("STARKNET").marketplace721`); `EKUBO_CORE_MAINNET` → `.ekuboCore`, etc. SDK-internal services (`registry`, `pop`, `drop`, `erc1155collection`, `creatorCoin`) resolve from the config's chain. `constants.ts` now holds only `SUPPORTED_TOKENS`/`DEFAULT_CURRENCY`. No behavior change — Starknet addresses are identical (they already derived from the registry). Audit + plan: `medialane-core/docs/specs/2026-06-23-platform-standardization-audit.md`, `medialane-core/docs/plans/2026-06-23-kill-mainnet-constant-debt.md`.
